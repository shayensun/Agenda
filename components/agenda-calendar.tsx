...
function formatReminderStatus(item: AgendaItem) {
  if (item.reminderSentAt) {
    return 'Sent';
  }

  if (item.reminderEnabled && item.reminderEmail) {
    return 'Armed';
  }
...
function normalizeAgendaItem(item: AgendaItem): AgendaItem {
  return {
    ...
    reminderError: item.reminderError ?? undefined,
  };
}
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      try {
        ...
        const results = (await response.json()) as {
          suggestions?: Array<{ displayName: string; lat: string; lon: string; subtitle?: string }>;
        };
        setLocationSuggestions(results.suggestions ?? []);
        setLocationStatus('idle');
      } catch (error) {
    ...
    const checkReminders = async () => {
      ...
      for (const item of agendaItems) {
        if (
          ...
          !item.reminderEmail ||
          item.reminderSentAt ||
          sendingReminderIds.current.has(item.id)
        ) {
          continue;
        }
            ...
            setAgendaItems((current) =>
              current.map((entry) =>
                entry.id === item.id
                  ? {
                      ...entry,
                      reminderError: (error as Error).message,
                    }
                  : entry,
              ),
            );
              ...
              <div className="mt-4 space-y-3">
                {selectedItems.length === 0 ? (
                  ...
                  selectedItems.map((item) => {
                    ...
                    const reminderStatus = formatReminderStatus(item);
                    return (
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="space-y-2">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-xs font-medium text-slate-400">Reminder: {reminderStatus}</span>
                            </div>
                            ...
                            {item.reminderError ? <p className="text-xs text-red-500">Reminder issue: {item.reminderError}</p> : null}
                          </div>
            ...
            <form onSubmit={handleSubmit} className="rounded-[1.75rem] border border-slate-200 bg-slate-50/80 p-4">
              ...
              <div className="mt-4 grid gap-4">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
                  <p className="font-semibold text-slate-700">Schedule preview</p>
                  <p className="mt-1">
                    {form.startTime ? formatTimeLabel(form.startTime) : 'No start time selected'}
                    {form.durationHours ? ` · ${formatDurationLabel(Number(form.durationHours)) || form.durationHours}` : ''}
                  </p>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-2 text-sm font-medium text-slate-700">
                    ...
                    <select
                      value={form.category}
                      onChange={(event: React.ChangeEvent<HTMLSelectElement>) =>
                        setForm((current) => ({ ...current, category: event.target.value as AgendaCategory }))
                      }
                      className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-slate-400"
                    >
                  ...
                  <label className="grid gap-2 text-sm font-medium text-slate-700">
                    ...
                    <input
                      ...
                      onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                        setForm((current) => ({ ...current, reminderEmail: event.target.value }))
                      }
                      className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition placeholder:text-slate-400 focus:border-slate-400"
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-2">
                    ...
                    <div className="space-y-1">
                      {locationSuggestions.map((suggestion) => (
                        ...
                        >
                          <span>
                            <span className="block">{suggestion.displayName}</span>
                            {suggestion.subtitle ? <span className="mt-1 block text-xs text-slate-400">{suggestion.subtitle}</span> : null}
                          </span>
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          {item.reminderEnabled && item.reminderEmail ? (
                            <p className="mt-1 text-xs text-amber-700">Reminder email: {item.reminderEmail}</p>
                          ) : null}
                        </div>
